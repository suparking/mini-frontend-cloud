...
Page({
        ...
        if (parks && parks.length > 0) {
          ...
          parks.forEach((park, index) => {
            let tPark = {
              ...
              address: park.addressSelect.split("-")[
                park.addressSelect.split("-").length - 1
              ],
  ...
  onMarkerTab(e) {
    ...
    const park = this.data.parkList.find(
      (item) => item.name === currentPark.title
    );
  ...
  popupParkDetail(e) {
    ...
    if (park.status === "OPENING") {
      ...
      let currentMarkerDetail = {
        ...
        latitude: park.location.latitude,
        longitude: park.location.longitude,
        iconPath: "../../assets/images/park-location-online.png",
        width: "64rpx",
        height: "64rpx",
        callout: {
          content: park.name,
